      ...
      {posts.length >= 1 &&
        posts
          ...
          .map((post, id) => {
            return (
              <div
                className="w-full flex flex-col p-2 justify-start items-center"
                key={id}
              >
                <img
                  src={post.postImage}
                  alt="Post Image"
                  className={`w-full h-[200px] md:h-[400px] object-cover md:rounded-lg shadow-md shadow-black mb-2`}
                />
                <h3 className="text-3xl font-bold">{post.title}</h3>
                <h4 className="text-gray-400">
                  Posted: {createDate(post.date)}
                </h4>
                <Link
                  href={{
